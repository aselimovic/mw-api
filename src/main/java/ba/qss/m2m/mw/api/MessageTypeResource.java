...
import ba.qss.m2m.mw.dao.MessageTypeDAO;
import ba.qss.m2m.mw.dao.MessageTypeTO;
...
public class MessageTypeResource {
    ...
    private static final String CLASS_NAME =
    		MessageTypeResource.class.getName();
    public static final Logger logger = LoggerFactory.getLogger(CLASS_NAME);

    @GET
//    @ValidateRequest
    public MessageTypeTO[] select(
    		/*@Context UriInfo uriInfo
			,*/@Pattern(regexp="[\\w\\s,]+") @Size(max=50) @DefaultValue("message_type_id ASC") @QueryParam("sort") String sort // XXX: Sort key expression: ORDER BY sin(i)
		...
		MessageTypeDAO messageTypeDAO = null;
		List<MessageTypeTO> messageTypes = null;
		MessageTypeTO criteria = new MessageTypeTO();
        IntValue rowCount = new IntValue(0);

		try {
			messageTypeDAO = OracleMWDAOFactory.getMessageTypeDAO();
			messageTypes = (List<MessageTypeTO>) (List) messageTypeDAO.select(
    	...
    	return messageTypes.toArray(new MessageTypeTO[messageTypes.size()]);
    }
}