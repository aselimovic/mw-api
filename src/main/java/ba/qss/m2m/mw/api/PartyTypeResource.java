...
import ba.qss.m2m.mw.dao.PartyTypeDAO;
import ba.qss.m2m.mw.dao.PartyTypeTO;
...
public class PartyTypeResource {
    ...
    private static final String CLASS_NAME = PartyTypeResource.class.getName();
...
//	@ValidateRequest
	public PartyTypeTO[] select(
			...
			,*/@Pattern(regexp="[\\w\\s,]+") @Size(max=50) @DefaultValue("party_type_id ASC") @QueryParam("sort") String sort // XXX: Sort key expression: ORDER BY sin(i)
			,@Min(value=0, message="{partyType.sort.size}") @Max(100) @DefaultValue("0") @QueryParam("pageIndex") int pageIndex
			,@Min(0) @Max(1000) @DefaultValue("20") @QueryParam("pageSize") int pageSize) {
		PartyTypeDAO partyTypeDAO = null;
		List<PartyTypeTO> partytipes = null;
        PartyTypeTO criteria = new PartyTypeTO();
		...
		try {
            ...
            partyTypeDAO = OracleMWDAOFactory.getPartyTypeDAO();
            partytipes = (List<PartyTypeTO>) (List) partyTypeDAO.select(criteria, PartyTypeDAO.SELECT_SQL_LIST, null/*filterExpression*/, sort, pageIndex, pageSize, rowCount);
            // DAOException
		...
		return partytipes.toArray(new PartyTypeTO[partytipes.size()]);
	}
	
	@GET
	@Path("partytype/{partyTypeId}")
	public PartyTypeTO findPartyTypeByPrimaryKey(@PathParam("partyTypeId") int partyTypeId) throws DAOException {
        PartyTypeDAO partyTypeDAO = null;
        PartyTypeTO partyTypeTO = null;
        PartyTypeTO criteria = new PartyTypeTO();
        criteria.setPartyTypeId(partyTypeId);

        try {
        	partyTypeDAO = OracleMWDAOFactory.getPartyTypeDAO();
        	partyTypeTO = (PartyTypeTO) partyTypeDAO.findByPrimaryKey(criteria, PartyTypeDAO.FIND_BY_PRIMARY_KEY_SQL);
        	// DAOException
        	
        	if (partyTypeTO == null) {throw new WebApplicationException(Response.Status.NOT_FOUND);
        	}
        } catch (DAOException e) {
        	logger.error(null, e);
        	throw e; // Rethrow
...
//	@ValidateRequest
	public PartyTypeTO create(@Valid PartyTypeTO newPartyTypeTO) {
        PartyTypeDAO partyTypeDAO = null;
        Object primaryColVal = null;

        try {
        	partyTypeDAO = OracleMWDAOFactory.getPartyTypeDAO();
        	primaryColVal = partyTypeDAO.create(newPartyTypeTO, PartyTypeDAO.INSERT_SQL);
        	newPartyTypeTO.setPartyTypeId(((Integer) primaryColVal).intValue());
        } 
        catch (DAOException e) {
		...
		return newPartyTypeTO;
	}
	
	@PUT
	public void update(PartyTypeTO partyTypeTO) {
		
		PartyTypeDAO partyTypeDAO = null;    
        try {
        	partyTypeDAO = OracleMWDAOFactory.getPartyTypeDAO();
        	partyTypeDAO.update(partyTypeTO, PartyTypeDAO.UPDATE_SQL);
        } 
        catch (DAOException e) {
	...
	@DELETE
	@Path("partytype/{partyTypeId}")
	public void delete(@PathParam("partyTypeId") int partyTypeId) {
        ...
        try {
        	...
        	if (partyTypeTO != null) {
        		partyTypeDAO.delete(partyTypeTO, PartyTypeDAO.DELETE_SQL);
        	} 
        	else {
