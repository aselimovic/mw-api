...
public class ReportingPeriodResource {
    ...
    @GET
//    @ValidateRequest
    public ReportingPeriodTO[] select(
    		/*@Context UriInfo uriInfo
			,*/@Pattern(regexp="[\\w\\s,]+") @Size(max=50) @DefaultValue("reporting_period_id ASC") @QueryParam("sort") String sort // XXX: Sort key expression: ORDER BY sin(i)
		...
		ReportingPeriodDAO reportingPeriodDAO = null;
		List<ReportingPeriodTO> reportingPeriods = null;
		ReportingPeriodTO criteria = new ReportingPeriodTO();
        IntValue rowCount = new IntValue(0);

		try {
			reportingPeriodDAO = OracleMWDAOFactory.getReportingPeriodDAO();
			reportingPeriods = (List<ReportingPeriodTO>) (List) reportingPeriodDAO.select(
					criteria, ReportingPeriodDAO.SELECT_SQL_LIST, null/*filterExpression*/,
    	...
    	return reportingPeriods.toArray(new ReportingPeriodTO[reportingPeriods.size()]);
    }
}
